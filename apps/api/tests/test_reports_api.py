...
def test_one_click_dossier_meets_acceptance(client):
    # um clique gera o dossiê...
    resp = client.post("/facilities/fx-joi-landfill/report", json={})
...
def test_dossier_text_carries_disclaimers_and_traceability(client):
    resp = client.post("/facilities/fx-cap-coal/report", json={"export_fraction_eu": 0.3})
    body = resp.json()
...
def test_report_for_unknown_facility_404(client):
    assert client.post("/facilities/nao-existe/report", json={}).status_code == 404
